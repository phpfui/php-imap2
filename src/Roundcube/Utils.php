...
class Utils
{
	...
	public static function check_email($email, $dns_check = true)
	{
		...
		if (\preg_match('/^\[((IPv6:[0-9a-f:.]+)|([0-9.]+))\]$/i', $domain_part, $matches)) {
			...
			if (0 !== \strpos($last_part, 'xn--') && \preg_match('/[^a-zA-Z]/', $last_part)) {
				return false;
			}
	...
	public static function file2class($mimetype, $filename)
	{
		...
		[$primary, $secondary] = \explode('/', $mimetype);
	...
	public static function get_opt($aliases = [])
	{
		...
		for ($i = 1; $i < \count($_SERVER['argv']); $i++) {
			...
			if ('-' == $arg[0]) {
				...
				elseif (\strlen($_SERVER['argv'][$i + 1]) && '-' != $_SERVER['argv'][$i + 1][0]) {
	...
	public static function https_check($port = null, $use_https = true)
	{
		...
		if (! empty($_SERVER['HTTP_X_FORWARDED_PROTO'])
			...
			&& \in_array($_SERVER['REMOTE_ADDR'], rcube::get_instance()->config->get('proxy_whitelist', []))
		) {
			return true;
		}
		...
		return (bool)($use_https && rcube::get_instance()->config->get('use_https'));
	}
		...
		while (($pos = \strpos($source, '{', $last_pos)) && ($pos2 = \strpos($source, '}', $pos))) {
			...
			if ($allow_remote) {
				...
				for ($i = 0, $len = \count($a_styles); $i < $len; $i++) {
					...
					elseif (\stripos($stripped, 'url(') && ! \preg_match($strict_url_regexp, $line)) {
						$a_styles = ['/* evil! */'];
	...
	public static function parse_host($name, $host = '')
	{
		...
		if (false !== \strpos($name, '%s')) {
			...
			if ($matches < 1 || false === \filter_var($s[1] . '@' . $s[2], FILTER_VALIDATE_EMAIL)) {
				return false;
			}
		}

		return \str_replace(['%n', '%t', '%d', '%h', '%z', '%s'], [$n, $t, $d, $h, $z, $s[2]], $name);
	}
	 ...
	 */
	public static function parse_socket_options(&$options, $host = null) : void
	{
	...
	public static function preg_error($error = [], $terminate = false)
	{
		if (($preg_error = \preg_last_error()) != PREG_NO_ERROR) {
			...
			if (PREG_BACKTRACK_LIMIT_ERROR == $preg_error) {
				$errstr .= ' Consider raising pcre.backtrack_limit!';
			}

			if (PREG_RECURSION_LIMIT_ERROR == $preg_error) {
	...
	public static function remote_addr()
	{
		...
		if (! empty($_SERVER['HTTP_X_FORWARDED_FOR']) || ! empty($_SERVER['HTTP_X_REAL_IP'])) {
			$proxy_whitelist = rcube::get_instance()->config->get('proxy_whitelist', []);
	...
	public static function rep_specialchars_output($str, $enctype = '', $mode = '', $newlines = true)
	{
		...
		if ('html' == $enctype) {
			...
			elseif ('strict' != $mode) {
				...
				if (false !== $ltpos && false !== \strpos($str, '>', $ltpos)) {
					unset($encode_arr['"'], $encode_arr['<'], $encode_arr['>'], $encode_arr['&']);
		...
		if ('text' == $enctype) {
			return \str_replace("\r\n", "\n", 'remove' == $mode ? \strip_tags($str) : $str);
		}
	...
	{
		$out = [];
		$src = self::INPUT_GET == $mode ? $_GET : (self::INPUT_POST == $mode ? $_POST : $_REQUEST);

		foreach (\array_keys($src) as $key) {
			$fname = '_' == $key[0] ? \substr($key, 1) : $key;
	...
	public static function resolve_url($url)
	{
		...
		if (! \preg_match('|^https?://|', $url)) {
			...
			if ($_SERVER['SERVER_PORT'] != $default_port && 80 != $_SERVER['SERVER_PORT']) {
	...
	public static function server_name($type = null, $strip_port = true)
	{
		...
		if (! empty($name)) {
			...
			$rcube->raise_error(['file' => __FILE__, 'line' => __LINE__,
	 ...
	 */
	public static function setcookie($name, $value, $exp = 0, $http_only = true) : void
	{
		if (\headers_sent()) {
			return;
		}
	...
	public static function words_match($haystack, $needle)
	{
		...
		foreach ($a_needle as $w) {
			if ($valid) {
				if (false !== \stripos($_haystack, $w)) {
					$hits++;
				}
			}
			elseif (false !== \stripos($haystack, $w)) {
				$hits++;
			}
		}
